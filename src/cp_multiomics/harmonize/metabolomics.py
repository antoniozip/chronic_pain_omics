"""Metabolomics harmonizer — MetaboLights datasets."""
...
# Technology → canonical ID space for metabolomics
_PLATFORM_TO_ID_SPACE = {
    "NMR": "HMDB",
    "GC-MS": "HMDB",
    "LC-MS": "ChEBI",
    "mass spectrometry": "ChEBI",
}


@register_harmonizer("metabolomics")
class MetabolomicsHarmonizer(OmicsHarmonizer):
    """Harmonizes MetaboLights metabolomics metadata.

    ID space: ChEBI (preferred for LC-MS) or HMDB (preferred for NMR/GC-MS).
    Effect size unit: SMD (standardized mean difference) across concentration values.
    """
    ...
    def _harmonize_one(self, raw: dict[str, Any], cfg: dict[str, Any]) -> HarmonizedRecord:
        ...
        id_space = _PLATFORM_TO_ID_SPACE.get(platform_canonical, "ChEBI")

        flags = self.flag_quality(raw, min_samples)
        if not species_canonical:
            flags.append("missing_species")
